...
function SidebarProvider({
  ...
  // Keyboard shortcut handled by the native menu (Cmd+B → menu:left_sidebar_open),
  // wired in __root.tsx. The built-in window listener was removed because it
  // conflicted with the native accelerators (and WKWebView reports Cmd+Shift+B
  // as key "b", so it fired on both shortcuts).
  ...
  return (
    ...
    >
      ...
      <div
        ...
        data-side={side}
        className={cn(
          "fixed top-(--header-height,0px) bottom-0 z-10 hidden h-[calc(100svh-var(--header-height, 0))] w-(--sidebar-width) transition-[left,right,width] duration-200 ease-linear data-[side=left]:left-0 data-[side=left]:group-data-[collapsible=offcanvas]:-left-(--sidebar-width) data-[side=right]:right-0 data-[side=right]:group-data-[collapsible=offcanvas]:-right-(--sidebar-width) md:flex",
...
function SidebarRail({ className, ...props }: React.ComponentProps<"button">) {
  ...
  return (
    <button
      ...
      className={cn(
        "absolute inset-y-0 z-20 hidden w-4 transition-all ease-linear group-data-[side=left]:-right-4 group-data-[side=right]:left-0 after:absolute after:inset-y-0 after:inset-s-0.5 after:w-0.5 hover:after:bg-sidebar-border sm:flex ltr:-translate-x-1/2 rtl:-translate-x-1/2",
